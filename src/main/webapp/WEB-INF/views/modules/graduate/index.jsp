		...
		$("#printf").droppable({
			...
			drop : function(event, ui) {
				...
				if (ui.helper.attr("id") == "Text") {
					...
					var fileName = "E:"+"//"+"upload" + ui.helper.attr("id") + ".png";
					//alert(fileName);
					var el = $("<div class='printComponents' onclick='checkClick(this)' tabindex='0' onmousedown='setIndex(event,this)'></div>");
		...
		<div id="catalog">
			...
			<div>
				<ul>
					...
					<div draggable="false">
						<small>行:</small> <input class="FormAcount" id="FormLineAcount"
							onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
							onafterpaste
                    "if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,
							'')}else{this.value=this.value.replace(/\D/g,'')}"> <br />
						<small>列:</small> <input class="FormAcount" id="FormColumnAcount"
							onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
							onafterpaste
                    "if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,
		...
		<div class="util" id="util">
			<div>
				...
				<div style="float:left;">
					<form>
						设置画布大小: 宽:<input id="printfWidth" size="10" placeholder="width"
							onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
							onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" />
						高:<input id="printfHeight" size="10" placeholder="height"
							onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"
							onafterpaste="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}" />
						<input id="submitPrintf" type="button" value="确认画布大小"
							onclick="setPrintfSize()" />
					</form>
				</div>
				...
				<div style="float:left;margin-left:2em">
					<form>
						设置背景颜色 : 设置透明 <select id="backgroundTransparent">
						...
						<input id="submitColorSelect" onclick="changeBackgroundColor()"
							type="button" value="确认背景">
					</form>
				</div>
			</div>
			...
			<div class="_util" style="float:left;">
				<form action="smartUploaddemo.jsp" enctype="multipart/form-data"
					...
					输入图片ID<input name="id" size="10" id="imgid" onkeyup="checkUnique()"
						...
						type="file" id="imagefile" required> <input
						id="inputSubmit" type="submit" value="上传文件"
						onclick="createNewElements()">
				</form>
			</div>
			...
			<div>
				...
				<div style="float: left;padding-left:2em">
					<form action="uploadTheServer.jsp" enctype="multipart/form-data"
						method="post" target="_blank">
						<input type="hidden" id="theTxt" name="thePrintfTxt"> <input
							type="hidden" id="thePng" name="thePrintfPng"> <input
							id="downId" name="thePrintfID" size="10" placeholder="输入唯一的ID"
							onmouseover="writeTxtAndPng()" required> <input
							id="uploadTxtAndPng" type="submit" onmouseover="writeTxtAndPng()"
							value="下载到服务器">
					</form>
				</div>
				<%--下载模板--%>
				<div style="float: left;padding-left: 0.2em">
					<input type="button" id="downloadPDF" onclick="print()"
						value="下载PDF" /> <input type="button" id="savePrintf"
						onclick="saveTxt()" value="保存" /> <input type="button"
						id="historicalModle" onclick="jump()" value="查看历史模板" />
				</div>
			</div>
			...
			<div class="_util" style="float:left;">
				...
				<input id="bold" type="button" value="加粗"
					style="height:24px;line-height:20px" /> <input id="italic"
					type="button" value="加斜" style="height:24px;line-height:20px" /> <input
					id="underline" type="button" value="下划线"
					style="height:24px;line-height:20px" /> <input id="textareaBorder"
					type="button" value="边框" style="height:24px;line-height:20px" /> <select
					id="fontfamily">
			...
			<div style="float:left;" style="width:auto;">
